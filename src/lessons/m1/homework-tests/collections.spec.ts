import { Employee } from "../snippets/types";
import {
  employees,
  shoppingList,
  shoppingDict,
...
describe.skip("Collection restructuring", () => {
  ...
  it("array2dict can turn an array into a dictionary (with `id` key)", () => {
    ...
    const array2dict = <T extends { id: PropertyKey }>(
      array: T[]
    ): Record<string, T> => Object.fromEntries(array.map((it) => [it.id, it]));
  ...
  it("dict2array can turn a dictionary (with `id` key) into an array", () => {
    ...
    const dict2array = <T extends object>(
      dict: Record<string, T>
    ): (T & { id: PropertyKey })[] => {
      return Object.entries<T>(dict).map((it) => ({
        id: isNaN(Number(it[0])) ? it[0] : Number(it[0]),
        ...it[1],
      }));
    };
    ...
    expect(shoppingData.find((e) => e.id == 611830716982)!.name).toEqual(
      "Beer"
    );
  });
});
