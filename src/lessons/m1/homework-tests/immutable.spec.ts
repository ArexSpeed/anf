...
describe("Immutable ES6 operations", () => {
  ...
  it("merge two objects", () => {
    ...
    const merge2objects = <T, K>(item: T, second: K): Record<string, T & K> =>
      Object.assign({}, { ...item, ...second });
  ...
  it("merging multiple objects", () => {
    ...
    const mergeManyObjects = <T extends object[]>(...item: T) =>
      Object.assign({}, ...item);
  ...
  it("strip dynamic attribute from objects", () => {
    ...
    expect(stripKey("qty", stripKey("price", shoppingList[0]))).toEqual({
      type: "Clothes",
      name: "Socks",
      id: 421801449988,
    });
