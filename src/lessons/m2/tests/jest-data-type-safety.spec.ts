...
describe("getTotalSalary", () => {
  ...
  const asProxy = <TAsserted extends TActual, TActual extends object = object>(
    t: TActual
  ) => {
    const proxy = new Proxy(t, {
      get: function (obj, prop) {
        if (!(prop in obj)) {
          throw new Error(
            `Trying to access non-existent property "${String(
              prop
            )}" on object ${JSON.stringify(obj)}`
          );
        }
  ...
  it.skip("should calculate sum of _filtered_ employees salaries (ES6 proxy)", () => {
    ...
    const result = getTotalSalary(
      mockEmployees.filter((e) => e.nationality == "PL")
    );
    expect(result).toEqual(300);
  });
});
